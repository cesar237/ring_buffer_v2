...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (i + 1 < argc) { // Make sure we have a value after the parameter
            if (strcmp(argv[i], "-p") == 0 || strcmp(argv[i], "--producers") == 0) {
                ...
                burst = atoi(argv[i + 1]);
                i++;
            } else if (strcmp(argv[i], "-n") == 0 || strcmp(argv[i], "--nr-rings") == 0) {
                nr_rings = atoi(argv[i + 1]);
                i++;
            }
        }
    }
    ...
    printf("  Pin threads: %s\n", PIN_THREADS ? "true" : "false");

    ring_buffer_t buffers[32];
    // Initialize the ring buffer
    for (int i = 0; i < 32; i++) {
        if (!ring_buffer_init_batch(&buffers[i], DEFAULT_BUFFER_SIZE, sizeof(test_item_t), 1)) {
    ...
    for (int i = 0; i < num_producers; i++) {
        ...
        producer_args[i].buffers = (ring_buffer_t *)buffers;
        producer_args[i].nr_rings = nr_rings;
    ...
    for (int i = 0; i < num_consumers; i++) {
        ...
        consumer_args[i].buffer = &buffers[ring];
        ring = (ring + 1) % nr_rings;
